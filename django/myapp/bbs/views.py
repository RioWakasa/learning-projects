...
def edit(request,id):
  article = get_object_or_404(Article, pk=id) #idで指定してarticleとして実体化
  articleForm = ArticleForm(instance=article) # ArticleFormにarticleに入っている情報を入れた状態で実体化
...
def create(request):
  if request.method == 'POST':
    articleForm = ArticleForm(request.POST) # ArticleFormの各カラムに入力値を入れた状態で実体化
    if articleForm.is_valid(): # 入力フォームに不備がないか
